// https://www.infoworld.com/article/3269057/14-excellent-reasons-to-use-f-sharp.html
...
let withdrawMoney amount =
    //SuccessfulWithdrawal(Amount = amount)
    UndisclosedFailure // Implementation elided
...
let doubleFunc = handleWithdrawal 100
...
module DiscriminatedUnions = 
    ...
    let card = { Suit = Hearts ; Rank = King }
    printfn $"{showPlayingCard card}"
...
module PatternMatching =
    ...
    let phil = Engineer { First="Philippe"; Last = "Laval" }
    let yvon = Engineer { First = "Yvon"; Last = "Filipecki" }
    let laurence = Engineer { First = "Laurence"; Last = "Rouget" }
    let marina = Manager( { First="Marina"; Last = "Laval" } , [phil; yvon] )
    let jeanlouis = Executive( { First="Jean-Louis"; Last="Rouget" } , [ marina ] , laurence)
...
and using an ad-hoc aggregate of values as a value itself.

Records are a "step up" from Tuples, having named labels and support for optional members.
They are great for a low-ceremony representation of data in-transit through your program.
Because they have structural equality, they are easy to use with comparison.

Discriminated Unions have many uses, but the core benefit is to be able to utilize them in conjunction with
Pattern Matching to account for all possible "shapes" that a data can have.

Classes are great for a huge number of reasons, such as when you need to represent information
and also tie that information to functionality. As a rule of thumb, when you have functionality that
is conceptually tied to some data, using Classes and the principles of Object-Oriented Programming is a
